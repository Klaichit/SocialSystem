...
| `eraseTarget(el,wide)` / `eraseNode(slot,el)` / `undoErase(slot)` | โหมดลบชิ้นส่วนในสไลด์ |
| `fieldsOf(slide)` / `buildCopyFields()` / `spread(role,v)` | แผงกรอกข้อความซ้ายที่ยืดตามเทมเพลต |
...
### แผงกรอกข้อความด้านซ้าย (copy editor)

ช่องกรอกไม่ได้ฟิกซ์ไว้ 5 ช่องแล้ว `fieldsOf(slide)` อ่าน `[contenteditable]` จริงในสไลด์ที่เลือก
ตามลำดับที่เห็นบนสไลด์ แล้ว `buildCopyFields()` สร้าง textarea เท่าจำนวนนั้น เทมเพลตที่มี
3 ชุดข้อความก็ได้ 3 ช่อง หัวข้อของช่องมาจากตาราง `FIELD_LABELS` (จับคู่จาก selector
ตัวแรกที่ตรง) ป้ายที่ซ้ำกันจะต่อเลขให้ เช่น `สเปก · ชื่อ 1/2/3`

**บล็อกใหม่ที่มีข้อความควรเพิ่ม selector ใน `FIELD_LABELS`** ไม่งั้นช่องจะขึ้นว่า `ข้อความ` เฉย ๆ

`role` ของช่อง = ป้าย + ลำดับ (`หัวข้อ#1`) ใช้เป็นกุญแจของ checkbox `ใช้กับทุกสไลด์ที่มีช่องเดียวกัน`
(`spread()` + `window.sharedCopy`) เขียนเฉพาะสไลด์ที่มี role ตรงกัน **ไม่สร้างบล็อกใหม่ให้สไลด์
ที่ไม่มีช่องนั้น** ถ้าไม่ติ๊ก จะแก้เฉพาะสไลด์ที่เลือก พิมพ์บนสไลด์ตรง ๆ ก็อัปเดตค่าในช่องซ้ายกลับมา
โดยไม่ rebuild (โฟกัสไม่หลุด) ส่วนการลบ/คืนชิ้นส่วนและลบสไลด์เรียก `window.refreshCopyFields()`
ให้รายการช่องตรงกับสไลด์เสมอ

ปุ่ม `แยกข้อความ` จับคู่ข้อความต้นฉบับกับช่องที่เทมเพลตนี้มีจริงเท่านั้น ช่องไหนไม่มีจะบอกไว้ใน
บรรทัดสถานะ ไม่ยัดบล็อกเพิ่มให้เอง

### ลบชิ้นส่วนในสไลด์ (erase mode)

ปุ่ม `ลบชิ้นส่วน` ในแถบของแต่ละสไลด์เปิดโหมดลบ (คลาส `erase` บน `.slot`) ชี้ที่ชิ้นไหน
ชิ้นนั้นจะขึ้นกรอบแดง คลิกแล้วหายไป กด Shift ค้างตอนคลิก = ลบทั้งกลุ่มที่ครอบอยู่
ปุ่ม `คืนชิ้นส่วน` คืนชิ้นล่าสุดจาก stack ที่เก็บไว้ใน `slot._erased` (ทีละใบ ไม่ปนกัน)
ปุ่ม `ลบชิ้นนี้` ใน `#tbar` ทำอย่างเดียวกันกับข้อความที่กำลังโฟกัสอยู่

สิ่งที่ลบได้กำหนดด้วยรายการ selector `ERASABLE` — **บล็อกใหม่ที่มีลูกซ้ำ ๆ ต้องเพิ่ม
selector ของลูกเข้าไปด้วย** ไม่งั้นจะลบได้เฉพาะทั้งก้อน

การลบมีผลกับ DOM เท่านั้น ไม่แตะ `templates.json` เพิ่มสไลด์ใบเดิมใหม่ก็ได้ครบเหมือนเดิม
ตัวจับคลิกอยู่ที่ capture phase บน `#work` เพราะต้องชิงก่อน caret ของ contenteditable,
file picker ของ `.mockup` และ drop zone ของสไลด์ และ `enableDrop()` มี guard
`_dropBound` ไม่ให้ผูก listener ซ้ำตอนคืนชิ้นส่วนกลับเข้ามา
