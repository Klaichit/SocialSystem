...
สคริปต์ท้ายไฟล์ (`/* ===== image analysis → template ===== */`) อ่านภาพอ้างอิงบน canvas
แล้วสร้าง object เทมเพลตขึ้นมาใหม่ UI เป็นหน้าต่างเต็มจอ `#anWin` ที่ยืมโครง `.gal` ของ
แคตตาล็อกมาใช้ (เปิด/ปิด/Escape/คลิกพื้นหลัง เหมือนกัน) ในแถบซ้ายเหลือแค่ปุ่มเปิด ทั้งหมดเป็นเรขาคณิตล้วน ไม่มี OCR ไม่เรียกเน็ต
ย่อภาพเหลือกว้าง 220px แล้วไล่:

1. `background(s)` — หาสีพื้นจากกรอบภาพ 3px คืนค่าสีกับ `even` (สัดส่วนของ bucket
   ที่ชนะ) ถ้า `even` ต่ำแปลว่ากรอบไม่นิ่ง = ภาพเต็มใบ
2. `mask(s,bg)` — พิกเซลที่ห่างจากพื้นเกิน `INK` (ผลรวมสามช่อง 62) นับเป็นเนื้อหา
3. `bands(s,mk)` — แถวที่มีเนื้อหาเกิน `ROW_ON` รวมเป็นบรรทัด แล้วรวมบรรทัดเป็นแถบ
   เมื่อระยะห่างไม่เกิน 2.2 เท่าของความสูงบรรทัด และขนาดบรรทัดใกล้กัน
4. `describe(s,mk,b)` — วัดต่อแถบ: ขอบซ้าย/ขวา, กลุ่มที่คั่นด้วย gutter กว้างเกิน 3.5%,
   ความหนาแน่น, ความหลากสี (`rich` = bucket 5 บิตที่ต่างกัน / ตัวอย่าง → แยกภาพจากข้อความ),
   จัดกลางไหม, มีสัญลักษณ์นำหน้าบรรทัดไหม
5. `toBlocks(...)` — แม็ปเป็นบล็อกที่มีอยู่แล้ว คืน `{blocks, classes, report}`
   และการันตีว่าปิดท้ายด้วย `foot` เสมอ

เทมเพลตที่ได้ถูกยัดเข้า `DATA.templates` กลุ่ม `ref` แล้วเรียก `indexData()` `buildPickers()`
ต่อ จึงติดไปกับ templates.json ที่ดาวน์โหลด **ถ้าเพิ่มบล็อกชนิดใหม่แล้วอยากให้ตัววิเคราะห์
เลือกได้ ต้องเพิ่มเงื่อนไขใน `toBlocks` เอง** ไม่งั้นจะไม่มีทางถูกเลือก
